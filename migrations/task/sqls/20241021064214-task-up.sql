...
values('7 堂組合包方案',7,1400),('14 堂組合包方案',14,2520),('21 堂組合包方案',21,4000);
...
(select credit_amount from "CREDIT_PACKAGE" where name = '14 堂組合包方案' ), (select price from "CREDIT_PACKAGE" where name = '14 堂組合包方案') ),
((select id from "USER" where name = '王小明'), (select id from "CREDIT_PACKAGE" where name = '21 堂組合包方案' ), 
(select credit_amount from "CREDIT_PACKAGE" where name = '21 堂組合包方案' ), (select price from "CREDIT_PACKAGE" where name = '21 堂組合包方案') ),
...
insert into "COACH"(user_id, experience_years, description)
values
((select id from "USER" where email = 'lee2000@hexschooltest.io'), 2, (select role from "USER" where email = 'lee2000@hexschooltest.io')),
((select id from "USER" where email = 'muscle@hexschooltest.io'), 2, (select role from "USER" where email = 'muscle@hexschooltest.io')),
((select id from "USER" where email = 'starplatinum@hexschooltest.io'), 2, (select role from "USER" where email = 'starplatinum@hexschooltest.io'));
...
where c.user_id = (select id from "USER" where name = '肌肉棒子');
-- 2. 教練`Q太郎` 的經驗年數為5年
update "COACH" c
set experience_years = 5
where c.user_id = (select id from "USER" where name = 'Q太郎');
...
where user_id  = (select id from "USER" where name = '王小明') and course_id = (select c.id from "COURSE"c inner join "USER"u on c.user_id = u.id where u.name = '李燕容');
...
select "a".name as 教練名稱, "a".experience_years as 經驗年數, s."name"  as 專長名稱
from
...
select s."name" as 專長名稱, count("a".id) as coach_total
from 
(select u.name,c.id,c.user_id,c.experience_years from "USER" u 
inner join "COACH" c
on u.id =c.user_id )as "a"
...
select cp."name" as 組合包方案名稱, count(cp2.credit_package_id ) as 銷售數量
...
-- 顯示須包含以下欄位： 總營收
select sum(cp.price_paid ) as 總營收
from  "CREDIT_PURCHASE" cp 
where cp.purchase_at between '2024-11-01 00:00:00' and '2024-11-30 23:59:59';
